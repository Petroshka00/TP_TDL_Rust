        ...
        for (entity, wants_melee, name, stats) in (&entities, &wants_melee, &names, &combat_stats).join() {
            if stats.hp > 0 {
                ...
                if target_stats.hp > 0 {
                    ...
                    let damage = i32::max(1, (stats.power + offensive_bonus) - (target_stats.defense + defensive_bonus));

                    if damage == 0 {
                        log.entries.push(format!("{} no puede dañar a {}", &name.name, &target_name.name));
                    } else {
